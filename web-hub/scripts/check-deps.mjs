...
async function ensureProxy(expectedBrowserId, browserOverride) {
  ...
  for (let i = 1; i <= 15; i++) {
    ...
    if (i === 1) console.log('⚠️  浏览器可能有授权弹窗，请点击「允许」后等待连接...');
...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    ...
    case 'ambiguous': {
      ...
      console.log('  请询问用户哪个浏览器作为默认（写入 config.env 的 WEB_ACCESS_BROWSER）');
    ...
    case 'mismatch': {
      ...
      const srcDesc = result.source === 'override' ? '本次指定' : '默认偏好';
      console.log(`browser: error — ${srcDesc}的浏览器是 "${expected}" (${expectedLabel})，但没连上`);
      console.log(`  请在 ${expectedLabel} 地址栏访问 ${expected}://inspect/#remote-debugging，勾选允许`);
      printAvailableHint(result.detected);
    ...
    case 'empty': {
      ...
      console.log('  在浏览器地址栏打开 chrome://inspect/#remote-debugging 或 edge://inspect/#remote-debugging，勾选允许');
      return { proceed: false, exitCode: 1 };
    }
  }
}
