...
# 质量档位政策（2026-09-03 用户拍板）：
#   默认 native（原生 50 步，文字渲染最好）；慢/排队多时回退 turbo_lora（均衡 9 步），
#   回退后必须查文字渲染（qa_check 底缘条带/抽帧）。
#   禁用 vsa（速度 4 步）与 turbo 别名（app.py select_backend 会把 turbo 路由到 vsa 速度引擎，
#   速度档文字渲染几乎完全不可用；09-03 shanghuo 批次实测中招——底缘烂字幕）。
ALLOWED_QUALITIES = ["native", "turbo_lora"]
...
def validate_line(obj: dict, default_quality: str = DEFAULT_QUALITY) -> list:
    errs = []
    ...
    q = obj.get("quality", default_quality)
    if q in ("vsa", "turbo"):
        errs.append(f"quality={q} 已禁用（速度档文字渲染几乎不可用；turbo 别名也会路由到 vsa 速度引擎），只用 {ALLOWED_QUALITIES}")
    elif q not in ALLOWED_QUALITIES:
        errs.append(f"quality 只支持 {ALLOWED_QUALITIES}（app.py:58）")
...
def build_payload(obj: dict, default_quality: str = DEFAULT_QUALITY) -> tuple:
    ...
    payload = {
        ...
        "aspect_ratio": obj["ar"],
        "task_type": obj["task"],
        "quality": obj.get("quality", default_quality),
    }
...
def submit_and_wait(client, idx, line, args, log_f, existing_tid=None) -> str:
    ...
    if existing_tid is None:
        payload, image_note = build_payload(line, default_quality=args.quality)
...
def main() -> int:
    ...
    ap.add_argument("--quality", default=DEFAULT_QUALITY, choices=ALLOWED_QUALITIES,
                    help="批次行未写 quality 时的默认档（默认 native 原生；太慢/排队多可回退 turbo_lora 均衡，回退后必须查文字渲染）")
    ...
    valid, invalid = [], []
    for i, obj in lines:
        errs = validate_line(obj, default_quality=args.quality)
        if errs:
            invalid.append(i)
