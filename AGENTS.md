...
自建 Codex / Codex skills 集合。本仓库是全量 skill 源码的唯一源头；`~/.agents/skills/` 是同 remote（evanmaranzano/Skills）的 sparse-checkout 工作副本，只跟踪 activitywatch-daily-log、last30days、web-hub 三个已安装 skill，`~/.Codex/skills/` 与 `~/.codex/skills/` 是指向 `~/.agents/skills/` 的软链接。其余 skill（lark-*、officecli、neat-freak 等）在本机 `.agents` 副本中被 .gitignore 排除，不在本仓库跟踪。
