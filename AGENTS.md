...
- 背景音乐（2026-09-12 从 video-judges 移植）：`assets/award-music.mp3` 存在时 base64 内嵌为 `<audio loop>`，加载即尝试播放、被拦截则首次点击/按键起播后一直循环；缺失时静默跳过。右下角有固定圆形金色播放/暂停按钮（`.bgm-toggle`，.playing 态切换音符/暂停图标），无音乐时按钮不渲染。
